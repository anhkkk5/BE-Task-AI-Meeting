...
class LinkWorkflowToProjectsAndTasks1786053000000 {
    ...
    async up(q) {
        ...
        for (const [suffix, key, label, color, category, order] of statuses)
            await q.query('INSERT INTO `workflow_statuses` (`id`,`template_id`,`status_key`,`label`,`color`,`category`,`sort_order`,`enabled`) VALUES (?,?,?,?,?,?,?,1)', [
        ...
        for (const [suffix, from, to] of transitions)
            await q.query('INSERT INTO `workflow_transitions` (`id`,`template_id`,`from_key`,`to_key`,`allowed_roles`) VALUES (?,?,?,?,NULL)', [
                `00000000-0000-4000-8000-0000000000${suffix}`,
                '00000000-0000-4000-8000-000000000001',
                from,
                to,
    ...
    async down(q) {
        await q.query('ALTER TABLE `tasks` DROP FOREIGN KEY `FK_task_workflow_status`');
        await q.query('ALTER TABLE `projects` DROP FOREIGN KEY `FK_project_workflow_template`');
