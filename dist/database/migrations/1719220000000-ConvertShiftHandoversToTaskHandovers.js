...
exports.ConvertShiftHandoversToTaskHandovers1719220000000 = void 0;
class ConvertShiftHandoversToTaskHandovers1719220000000 {
    name = 'ConvertShiftHandoversToTaskHandovers1719220000000';
    async up(queryRunner) {
        await queryRunner.query('ALTER TABLE `shift_handovers` DROP FOREIGN KEY `FK_handovers_source_shift`');
    ...
    async down(queryRunner) {
        await queryRunner.query("UPDATE `shift_handovers` SET `status` = 'CANCELLED' WHERE `status` = 'REJECTED'");
        await queryRunner.query('ALTER TABLE `shift_handovers` DROP FOREIGN KEY `FK_handovers_task`');
        await queryRunner.query('ALTER TABLE `shift_handovers` DROP INDEX `IDX_handovers_task`');
    ...
    `);
        await queryRunner.query("ALTER TABLE `shift_handovers` MODIFY COLUMN `status` enum('DRAFT','PENDING','CHANGES_REQUESTED','ACKNOWLEDGED','CANCELLED') NOT NULL DEFAULT 'DRAFT'");
        await queryRunner.query('CREATE INDEX `IDX_handovers_source_shift` ON `shift_handovers` (`source_shift_id`)');
        await queryRunner.query('ALTER TABLE `shift_handovers` ADD CONSTRAINT `FK_handovers_source_shift` FOREIGN KEY (`source_shift_id`) REFERENCES `shifts`(`id`) ON DELETE CASCADE');
        await queryRunner.query('ALTER TABLE `shift_handovers` ADD CONSTRAINT `FK_handovers_target_shift` FOREIGN KEY (`target_shift_id`) REFERENCES `shifts`(`id`) ON DELETE SET NULL');
    }
}
exports.ConvertShiftHandoversToTaskHandovers1719220000000 = ConvertShiftHandoversToTaskHandovers1719220000000;
//# sourceMappingURL=1719220000000-ConvertShiftHandoversToTaskHandovers.js.map