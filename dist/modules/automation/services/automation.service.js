...
let AutomationService = class AutomationService {
    ...
    async list(userId, workspaceId, projectId) {
    ...
    async save(userId, workspaceId, projectId, dto, id) {
        await this.access(userId, workspaceId, projectId);
        this.validate(dto);
        ...
        if (enabled && !dryRunAt)
            throw new common_1.BadRequestException('Run a dry-run before enabling this rule');
    ...
    async history(userId, workspaceId, projectId, id) {
        await this.access(userId, workspaceId, projectId);
    ...
    async runRule(rule, forceRetry = 0) {
        const matches = await this.matches(rule);
        const results = [];
    ...
    async retry(userId, workspaceId, projectId, runId) {
        await this.access(userId, workspaceId, projectId);
        const previous = await this.repo.findRun(runId);
        if (!previous || previous.status !== 'FAILED')
            throw new common_1.BadRequestException('Only failed runs can be retried');
            ...
            for (const action of rule.actions) {
                if (action.type === 'NOTIFY_ASSIGNEE' && task.assigneeId) {
                    await this.notifications.create({
                        ...
                        type: notification_entity_1.NotificationType.TaskDueSoon,
                        title: 'Nhắc việc tự động',
                        body: action.message || `${task.taskCode} - ${task.title} sắp đến hạn`,
    ...
    async matches(rule) {
        const date = new Date(Date.now() + Math.max(0, Number(rule.trigger.daysBefore ?? 0)) * 86400000)
            .toISOString()
            .slice(0, 10);
        const candidates = await this.tasksRepo.findDueNotificationCandidates(date);
        return candidates.filter((task) => task.projectId === rule.projectId &&
    ...
    validate(dto) {
        if (dto.trigger.type !== 'DUE_DATE')
            throw new common_1.BadRequestException('Only DUE_DATE trigger is supported');
        if (!dto.actions.length || dto.actions.length > 5)
            throw new common_1.BadRequestException('Rule must contain 1-5 actions');
