    ...
    async summary(hours = 24) {
        ...
        return {
            ...
            totals: {
                ...
                slowApis: items.filter((item) => item.kind === 'API' && item.status === 'SLOW').length,
                failedJobs: items.filter((item) => item.kind === 'SCHEDULER' && item.status === 'FAILED').length,
                failedEmails: items.filter((item) => item.kind === 'EMAIL' && item.status === 'FAILED').length,
            },
            ai: {
                calls: ai.length,
                inputTokens: ai.reduce((sum, item) => sum + (item.inputTokens ?? 0), 0),
                outputTokens: ai.reduce((sum, item) => sum + (item.outputTokens ?? 0), 0),
                estimatedCostUsd: ai.reduce((sum, item) => sum + Number(item.estimatedCostUsd ?? 0), 0),
                averageLatencyMs: ai.length
                    ? Math.round(ai.reduce((sum, item) => sum + (item.durationMs ?? 0), 0) /
                        ai.length)
                    : 0,
            },
