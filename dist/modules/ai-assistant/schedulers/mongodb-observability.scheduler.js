...
let MongodbObservabilityScheduler = class MongodbObservabilityScheduler {
    ...
    async check() { const started = Date.now(); try {
        ...
        await this.observability.record({ kind: 'MONGODB', status: 'SUCCESS', operation: 'mongodb.ping', durationMs: Date.now() - started, error: null, metadata: { readyState: this.connection.readyState } });
    }
    catch (error) {
        await this.observability.record({ kind: 'MONGODB', status: 'FAILED', operation: 'mongodb.ping', durationMs: Date.now() - started, error: error instanceof Error ? error.message : String(error), metadata: { readyState: this.connection.readyState } });
    } }
};
