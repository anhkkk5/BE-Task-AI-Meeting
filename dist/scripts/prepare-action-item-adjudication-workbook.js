...
async function readWorkbook(path) {
    if (!(0, node_fs_1.existsSync)(path))
        throw new Error(`Không tìm thấy workbook: ${path}`);
    ...
    sheet.eachRow((row, rowNumber) => {
        ...
        try {
            ...
            throw new Error(`${path}: JSON của ${transcriptId} không hợp lệ: ${error instanceof Error ? error.message : String(error)}`);
        }
        rows.push({
            transcriptId,
...
async function main() {
    ...
    const outputPath = (0, node_path_1.resolve)(process.env.ANNOTATION_ADJUDICATION_PATH ?? (0, node_path_1.join)(base, 'adjudication.xlsx'));
    ...
    for (const rowA of rowsA) {
        ...
        if (!rowB)
            throw new Error(`Annotator B thiếu ${rowA.transcriptId}.`);
        const identical = canonicalJson(rowA.actionItemsJson) ===
    ...
    console.log(`Đã tạo workbook phân xử ${rowsA.length} transcript tại ${outputPath}`);
    console.log('Mọi dòng đang PENDING; người phân xử phải đổi sang APPROVED thủ công.');
}
main().catch((error) => {
    console.error(error instanceof Error ? error.message : error);
    process.exitCode = 1;
});
//# sourceMappingURL=prepare-action-item-adjudication-workbook.js.map