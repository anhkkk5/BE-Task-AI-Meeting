...
async function main() {
    ...
    if (!projectId) {
        throw new Error('Usage: npx ts-node src/scripts/seed-project-sprints-backlog.ts <projectId>');
    }
    ...
    try {
        ...
        const [projects] = await db.execute(`SELECT id, workspace_id, created_by, workflow_template_id
       FROM projects WHERE id = ? AND deleted_at IS NULL LIMIT 1`, [projectId]);
        if (!projects.length)
            throw new Error(`Project not found: ${projectId}`);
        const project = projects[0];
        const [members] = await db.execute(`SELECT wm.user_id FROM workspace_members wm
       WHERE wm.workspace_id = ? AND wm.status = 'ACTIVE'
       ORDER BY wm.joined_at ASC, wm.created_at ASC`, [project.workspace_id]);
        if (!members.length)
            throw new Error('Workspace has no active members');
        const [workflowRows] = await db.execute(`SELECT id, status_key FROM workflow_statuses
       WHERE template_id = ? AND enabled = 1`, [project.workflow_template_id]);
        const workflowIds = new Map(workflowRows.map((row) => [String(row.status_key), String(row.id)]));
        for (const status of [
            'BACKLOG',
            'TODO',
            'IN_PROGRESS',
        ...
        const [existing] = await db.execute(`SELECT COUNT(*) AS total FROM tasks
       WHERE project_id = ? AND task_code LIKE ? AND deleted_at IS NULL`, [projectId, `${TASK_PREFIX}%`]);
        ...
        for (const sprint of sprintDefinitions) {
            ...
            await db.execute(`INSERT INTO sprints
        ...
        await db.commit();
        const [summary] = await db.execute(`SELECT COALESCE(s.name, 'Backlog chưa gán Sprint') AS bucket,
       ...
       GROUP BY s.name, t.status ORDER BY s.name, t.status`, [projectId, `${TASK_PREFIX}%`]);
...
async function insertTask(input) {
    await input.db.execute(`INSERT INTO tasks
...
//# sourceMappingURL=seed-project-sprints-backlog.js.map