...
const outputDirectory = (0, path_1.join)(backendRoot, 'datasets', 'functional-testing', 'v1');
function collectFiles(directory) {
    if (!(0, fs_1.existsSync)(directory))
        return [];
    return (0, fs_1.readdirSync)(directory, { withFileTypes: true }).flatMap((entry) => {
...
function inspect(filePath) {
    const source = (0, fs_1.readFileSync)(filePath, 'utf8');
    const normalizedPath = (0, path_1.relative)(backendRoot, filePath).replace(/\\/g, '/');
    const e2e = filePath.endsWith('.e2e-spec.ts');
    const rbacPattern = /WorkspaceRolesGuard|WorkspaceRole|ForbiddenException|\bRBAC\b|\b403\b/i;
    const tenantPattern = /multi[- ]?tenan|cross[- ]?workspace|another tenant|another workspace|other workspace|workspace boundaries|workspace-b|not found in this workspace|khong tron cache giua cac workspace/i;
    return {
        path: normalizedPath,
...
const evidence = [
    ...collectFiles((0, path_1.join)(backendRoot, 'src')),
    ...collectFiles((0, path_1.join)(backendRoot, 'test')),
]
    .map(inspect)
...
const totals = {
    ...
    methodology: {
        ...
        integrationE2e: 'File *.e2e-spec.ts trong test; khoi tao Nest TestingModule va kiem tra luong HTTP/module.',
        rbac: 'File co guard, WorkspaceRole, ForbiddenException/RBAC/HTTP 403.',
        multiTenancy: 'File co assertion ve bien workspace/tenant; danh sach file duoc cong khai de kiem tra thu cong.',
    },
...
if (totals.unit.files === 0 ||
    totals.integrationE2e.files === 0 ||
    totals.rbac.files === 0 ||
    totals.multiTenancy.files === 0) {
    throw new Error('Functional evidence is missing at least one required category.');
}
(0, fs_1.mkdirSync)(outputDirectory, { recursive: true });
(0, fs_1.writeFileSync)((0, path_1.join)(outputDirectory, 'functional-test-evidence.json'), `${JSON.stringify({ summary: totals, files: evidence }, null, 2)}\n`, 'utf8');
const securityRows = evidence
    .filter((item) => item.rbac || item.multiTenancy)
    .map((item) => `| \`${item.path}\` | ${item.testCases} | ${item.rbac ? 'Co' : '-'} | ${item.multiTenancy ? 'Co' : '-'} |`)
    .join('\n');
...
`;
(0, fs_1.writeFileSync)((0, path_1.join)(outputDirectory, 'README.md'), markdown, 'utf8');
console.log(JSON.stringify(totals, null, 2));
//# sourceMappingURL=generate-functional-test-evidence.js.map