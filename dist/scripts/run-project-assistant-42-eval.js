...
function generateAssistantResponse(c) {
...
function verifyCase(c, answer, sources) {
    ...
    const forbiddenViolated = c.groundTruth.forbiddenClaims.filter(claim => {
        if (claim.startsWith('Workspace khác') && answer.includes('workspace-')) {
            ...
            if (match && !claim.includes(match[0]))
                return true;
        }
        return false;
    });
...
function run() {
    ...
    const cases = rawLines.map((line, idx) => {
    ...
    const outputPath = (0, node_path_1.resolve)('datasets', 'project-assistant-evaluation', 'v1', 'evaluation_result_42_questions.json');
    (0, node_fs_1.mkdirSync)((0, node_path_1.dirname)(outputPath), { recursive: true });
    (0, node_fs_1.writeFileSync)(outputPath, JSON.stringify(outputSummary, null, 2), 'utf8');
...
run();
//# sourceMappingURL=run-project-assistant-42-eval.js.map