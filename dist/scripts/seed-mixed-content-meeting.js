...
const data_source_1 = __importDefault(require("../database/mysql/data-source"));
const mongodb_config_1 = require("../config/mongodb.config");
const meeting_transcript_schema_1 = require("../modules/meetings/schemas/meeting-transcript.schema");
const workspaceId = process.env.SEED_MEETING_WORKSPACE_ID ??
    'efef3f78-e908-4bb4-93cf-2f9e70f8cdff';
const projectId = process.env.SEED_MEETING_PROJECT_ID ??
    '5ef9db11-d6eb-4406-9e69-03b91a4b2d19';
const marker = '[AI_SUMMARY_MIXED_CONTENT_TEST]';
async function main() {
    const mongo = (0, mongodb_config_1.mongodbConfig)();
    if (!mongo.enabled)
        throw new Error('MONGODB_ENABLED must be true');
    await data_source_1.default.initialize();
    const projectRows = await data_source_1.default.query('SELECT id, name, workspace_id FROM projects WHERE id = ? AND workspace_id = ? AND deleted_at IS NULL LIMIT 1', [projectId, workspaceId]);
    if (!projectRows.length)
        throw new Error('Target project was not found');
    const members = await data_source_1.default.query(`SELECT wm.user_id, wm.role, u.full_name, u.email
     ...
     LIMIT 4`, [workspaceId]);
    if (!members.length)
        throw new Error('No active workspace member was found');
    const host = members[0];
    const teammate = members[1] ?? members[0];
    const title = `${marker} Họp rà soát tiến độ và chuẩn bị phát hành`;
    const existing = await data_source_1.default.query('SELECT id FROM meetings WHERE project_id = ? AND title = ? AND deleted_at IS NULL LIMIT 1', [projectId, title]);
    if (existing.length) {
        console.log(JSON.stringify({ meetingId: existing[0].id, reused: true }));
        return;
    }
    const meetingId = (0, node_crypto_1.randomUUID)();
    const now = new Date();
    ...
    const connection = await mongoose_1.default.createConnection(mongo.uri).asPromise();
    const TranscriptModel = connection.model(meeting_transcript_schema_1.MeetingTranscript.name, meeting_transcript_schema_1.MeetingTranscriptSchema);
    ...
    try {
        ...
        await data_source_1.default.transaction(async (manager) => {
            await manager.query(`INSERT INTO meetings
         ...
         VALUES (?, ?, ?, NULL, ?, ?, 'GENERAL', ?, ?, ?, ?, ?, 0,
            ...
            for (const member of members.slice(0, 2)) {
                await manager.query(`INSERT INTO meeting_participants
        ...
        if (transcriptId)
            await TranscriptModel.deleteOne({ _id: transcriptId });
        throw error;
    }
    finally {
...
//# sourceMappingURL=seed-mixed-content-meeting.js.map