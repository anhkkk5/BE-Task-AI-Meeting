...
async function main() {
    ...
    try {
        ...
        for (let w = 0; w < workspaceNames.length; w += 1) {
         ...
         VALUES (?,?,?,?,?,'FREE','ACTIVE',NOW(),NOW())`, [workspaceId, `${DEMO_TAG} ${workspaceNames[w]}`, `demo-data-${w + 1}`, 'Không gian dữ liệu mẫu đầy đủ để kiểm thử giao diện.', ownerId]);
           ...
           VALUES (?,?,?,?,'ACTIVE',NOW(),?,JSON_ARRAY(),NOW(),NOW())`, [(0, crypto_1.randomUUID)(), workspaceId, memberIds[m], m === 0 ? 'OWNER' : m === 1 ? 'PROJECT_MANAGER' : m === 2 ? 'SCRUM_MASTER' : 'MEMBER', 7 + (m % 3) * 0.5]);
            }
            for (let p = 0; p < 2; p += 1) {
                projectCount += 1;
           ...
           VALUES (?,?,?,?,?,'ACTIVE',?,?,NULL,NULL,?,?,NOW(),NOW())`, [projectId, workspaceId, `${DEMO_TAG} ${p ? 'Cổng quản trị' : 'Ứng dụng khách hàng'} ${w + 1}`, key, 'Dự án mẫu có đầy đủ sprint, task, cuộc họp và báo cáo hằng ngày.', sqlDate(date(-45)), sqlDate(date(60)), workflowTemplateId, ownerId]);
                const sprintIds = [];
                for (let s = 0; s < 3; s += 1) {
                    sprintCount += 1;
             ...
             VALUES (?,?,?,?,?,?,?,${s === 2 ? 'NULL' : '?'},${s === 0 ? '?' : 'NULL'},?,NOW(),NOW())`, [sprintId, projectId, `${DEMO_TAG} Sprint ${s + 1}`, `Hoàn thành nhóm chức năng giai đoạn ${s + 1}`, status, sqlDate(date(startOffset)), sqlDate(date(startOffset + 13)), ...(s === 2 ? [] : [date(startOffset)]), ...(s === 0 ? [date(startOffset + 13)] : []), ownerId]);
                }
                const taskIds = [];
                ...
                for (let t = 0; t < 36; t += 1) {
                    ...
                    const activeStatuses = ['DONE', 'DONE', 'IN_PROGRESS', 'REVIEW', 'TODO', 'TODO'];
             ...
             VALUES (?,?,?,?,?,?,?, ?,?,?, 'TASK',?,NULL,?,?,?, ?,?,NULL,?,?,?,NOW())`, [taskId, projectId, sprintIds[sprintIndex], `${key}-${t + 1}`, `${titles[t % titles.length]} #${Math.floor(t / titles.length) + 1}`, 'Công việc demo dùng để kiểm thử đầy đủ các màn hình.', JSON.stringify(['demo', t % 2 ? 'backend' : 'frontend']), 'Đáp ứng yêu cầu và được kiểm thử.', status, workflow.get(status), t % 9 === 0 ? 'URGENT' : t % 4 === 0 ? 'HIGH' : 'MEDIUM', memberIds[1 + (t % (memberIds.length - 1))], ownerId, ownerId, sqlDate(date((t % 14) - 4)), 2 + (t % 6), completedAt, status === 'BACKLOG' || status === 'TODO' ? null : date(-12 + (t % 6)), date(-25 + (t % 12))]);
                }
                for (let m = 0; m < 8; m += 1) {
                    meetingCount += 1;
             ...
             VALUES (?,?,?,?,?,?,?,?,?,?,?,?,0,?,?,NULL,NULL,NOW(),NOW())`, [meetingId, workspaceId, projectId, sprintIds[1], `${DEMO_TAG} ${type.replaceAll('_', ' ')} ${m + 1}`, 'Cuộc họp mẫu của nhóm dự án.', type, sqlDate(date(m - 5)), date(m - 5), date(m - 5 + 1 / 24), meetingStatus === 'COMPLETED' ? date(m - 5) : null, meetingStatus === 'COMPLETED' ? date(m - 5 + 1 / 24) : null, meetingStatus, ownerId]);
                    for (let u = 0; u < Math.min(5, memberIds.length); u += 1)
                        await db.execute(`INSERT INTO meeting_participants (id,meeting_id,user_id,role,attended,created_at,updated_at) VALUES (?,?,?,? ,?,NOW(),NOW())`, [(0, crypto_1.randomUUID)(), meetingId, memberIds[u], u === 0 ? 'HOST' : 'PARTICIPANT', meetingStatus === 'COMPLETED' ? 1 : 0]);
                }
                for (let d = 0; d < 7; d += 1)
                    for (let u = 0; u < Math.min(5, memberIds.length); u += 1)
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?, ?,NOW(),NOW())`, [(0, crypto_1.randomUUID)(), workspaceId, projectId, sprintIds[1], memberIds[u], sqlDate(date(-d)), 'Đã hoàn thành các công việc được giao.', 'Tiếp tục xử lý task trong Sprint.', d === 2 && u === 2 ? 'Đang chờ xác nhận yêu cầu.' : null, d === 2 && u === 2 ? memberIds[1] : null, `${DEMO_TAG} cập nhật hằng ngày`, d === 2 && u === 2 ? 'BLOCKED' : d % 3 === 0 ? 'GOOD' : 'NORMAL']);
                for (let h = 0; h < 4; h += 1)
       ...
       VALUES (?,?,?,?,?,?,?, ?,?,NULL,?)`, [(0, crypto_1.randomUUID)(), recipientId, ['TASK_ASSIGNED', 'TASK_DUE_SOON', 'MEETING_INVITED'][n % 3], `${DEMO_TAG} Thông báo ${n + 1}`, 'Thông báo dữ liệu mẫu để kiểm thử trung tâm thông báo.', '/dashboard', JSON.stringify({ demo: true }), `full-demo-${recipientId}-${n}`, n < 2 ? date(-n) : null, date(-n)]);
        await db.commit();
