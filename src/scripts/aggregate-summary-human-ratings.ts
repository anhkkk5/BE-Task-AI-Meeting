...
async function readRatings(path: string): Promise<Rating[]> {
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.readFile(path);
  const sheet = workbook.getWorksheet('Đánh giá');
  if (!sheet) throw new Error(`Không tìm thấy sheet "Đánh giá" trong ${path}`);
  const rows: Rating[] = [];
  sheet.eachRow((row, rowNumber) => {
    ...
    const scores = [7, 8, 9, 10, 11].map((column) =>
    ...
    if (!Number.isInteger(hallucinationCount) || hallucinationCount < 0) {
      throw new Error(
        `${path}: ${transcriptId} chưa nhập hallucinationCount hợp lệ.`,
      );
    }
    rows.push({
      transcriptId,
...
function summarize(rows: Rating[]) {
  return {
    ...
    hallucinationTotal: rows.reduce(
      (sum, row) => sum + row.hallucinationCount,
      0,
    ),
    hallucinationFreeRate: round(
      rows.filter((row) => row.hallucinationCount === 0).length / rows.length,
    ),
  };
}

async function main() {
  const base = join('datasets', 'summary-evaluation', 'v1');
  const aPath = resolve(
    process.env.SUMMARY_EVALUATOR_A_PATH ??
      join(base, 'human-rating', 'evaluator-a.xlsx'),
  );
  const bPath = resolve(
    process.env.SUMMARY_EVALUATOR_B_PATH ??
      join(base, 'human-rating', 'evaluator-b.xlsx'),
  );
  const outputPath = resolve(
    process.env.SUMMARY_EVAL_RESULT_PATH ??
  ...
  const combined = a.flatMap((row) => {
    ...
    if (!other) throw new Error(`Evaluator B thiếu ${row.transcriptId}.`);
    return [row, other];
  });
  ...
  const report = {
    ...
    byMeetingType: Object.fromEntries(
      types.map((type) => [
        type,
    ...
    interpretation:
      'Điểm là trung bình của hai người đánh giá độc lập trên thang 1..5.',
  };
