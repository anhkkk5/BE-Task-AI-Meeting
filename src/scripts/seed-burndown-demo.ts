...
async function main() {
  ...
  try {
    ...
    const [projects] = await connection.execute<RowDataPacket[]>(
      'SELECT id, workspace_id, workflow_template_id FROM projects WHERE id = ? AND deleted_at IS NULL LIMIT 1',
      [projectId],
    );
    ...
    const [workflowStatuses] = await connection.execute<RowDataPacket[]>(
    ...
    const [existingSprints] = await connection.execute<RowDataPacket[]>(
      'SELECT id FROM sprints WHERE project_id = ? AND name = ? AND deleted_at IS NULL LIMIT 1',
      [projectId, DEMO_SPRINT_NAME],
    );
    const sprintId = existingSprints.length
      ? String(existingSprints[0].id)
      : randomUUID();

    // Only one sprint should be active in the demo project.
    await connection.execute(
      `UPDATE sprints SET status = 'PLANNED', started_at = NULL
    ...
    if (existingSprints.length) {
      await connection.execute(
        `UPDATE sprints
         ...
         WHERE id = ?`,
        [toSqlDate(startDate), toSqlDate(endDate), startDate, sprintId],
      );
    } else {
      await connection.execute(
        `INSERT INTO sprints
    ...
    const [existingTasks] = await connection.execute<RowDataPacket[]>(
      'SELECT COUNT(*) AS total FROM tasks WHERE project_id = ? AND task_code LIKE ? AND deleted_at IS NULL',
      [projectId, `${DEMO_TASK_PREFIX}%`],
    );
    ...
    if (Number(existingTasks[0].total) === 0) {
      ...
      await connection.execute(
        'UPDATE tasks SET sprint_id = ? WHERE project_id = ? AND task_code LIKE ? AND deleted_at IS NULL',
        [sprintId, projectId, `${DEMO_TASK_PREFIX}%`],
      );
    }
