...
async function main() {
  ...
  try {
    ...
    const [statusRows] = await db.execute<RowDataPacket[]>(
      'SELECT id,status_key FROM workflow_statuses WHERE template_id=? AND enabled=1', [templateId],
    );
    const workflow = new Map(statusRows.map((row) => [String(row.status_key), String(row.id)]));
    await db.beginTransaction();
    let workspaces = 0; let memberships = 0; let projects = 0; let sprints = 0; let tasks = 0;
    ...
    for (let ownerIndex = 0; ownerIndex < users.length; ownerIndex += 1) {
      ...
      for (let workspaceIndex = 0; workspaceIndex < 2; workspaceIndex += 1) {
        ...
        const [existing] = await db.execute<RowDataPacket[]>('SELECT id FROM workspaces WHERE slug=? LIMIT 1', [slug]);
        ...
        await db.execute(
          ...
          [workspaceId, `[Demo Network] ${users[ownerIndex].full_name} - Nhóm ${workspaceIndex + 1}`, slug, 'Workspace mẫu có thành viên được phân bổ chéo từ danh sách user demo.', ownerId],
        );
        ...
        for (let position = 0; position < selected.length; position += 1) {
          ...
          await db.execute(
            ...
            [randomUUID(), workspaceId, userId, position === 0 ? 'OWNER' : position === 1 ? 'PROJECT_MANAGER' : position === 2 ? 'SCRUM_MASTER' : 'MEMBER', 7 + (position % 3) * 0.5],
          );
          memberships += 1;
        }
        ...
        await db.execute(
          ...
          [projectId, workspaceId, `[Demo] Dự án ${key}`, key, 'Dự án có backlog và công việc riêng trong từng Sprint.', sqlDate(-35), sqlDate(70), templateId, ownerId],
        );

        const sprintIds: string[] = [];
        for (let sprintIndex = 0; sprintIndex < 3; sprintIndex += 1) {
          const sprintId = randomUUID(); sprintIds.push(sprintId); sprints += 1;
          ...
          await db.execute(
            ...
            [sprintId, projectId, `[Demo] Sprint ${sprintIndex + 1}`, `Hoàn thành nhóm tính năng số ${sprintIndex + 1}`, status, sqlDate(start), sqlDate(start + 13), ...(sprintIndex === 2 ? [] : [at(start)]), ...(sprintIndex === 0 ? [at(start + 13)] : []), ownerId],
          );
        }
        ...
        for (let sprintIndex = 0; sprintIndex < sprintIds.length; sprintIndex += 1) {
          for (let taskIndex = 0; taskIndex < 15; taskIndex += 1) {
            const activeStatuses = ['DONE', 'IN_PROGRESS', 'REVIEW', 'TODO', 'BACKLOG'] as const;
