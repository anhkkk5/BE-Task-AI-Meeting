...
async function main() {
  ...
  try {
    const [projects] = await db.execute<RowDataPacket[]>(
      `SELECT id, workspace_id, created_by FROM projects
       WHERE id = ? AND deleted_at IS NULL LIMIT 1`,
      [PROJECT_ID],
    );
    if (!projects.length) throw new Error(`Project not found: ${PROJECT_ID}`);
    const project = projects[0];

    const [members] = await db.execute<RowDataPacket[]>(
      `SELECT wm.user_id, u.full_name, u.email
       FROM workspace_members wm
       INNER JOIN users u ON u.id = wm.user_id
       WHERE wm.workspace_id = ? AND wm.status = 'ACTIVE'
       ORDER BY CASE WHEN u.full_name IN ('nta', 'abcd2') THEN 0 ELSE 1 END, wm.created_at ASC`,
      [project.workspace_id],
    );
    if (members.length < 2) throw new Error('At least two active members are required');
    const selectedMembers = members.slice(0, 2);
    const userByName = new Map(
      selectedMembers.map((member) => [String(member.full_name), String(member.user_id)]),
    );

    const [existingMeetings] = await db.execute<RowDataPacket[]>(
      `SELECT id FROM meetings WHERE project_id = ? AND title = ? AND deleted_at IS NULL LIMIT 1`,
      [PROJECT_ID, MEETING_TITLE],
    );
    const meetingId = existingMeetings.length ? String(existingMeetings[0].id) : randomUUID();
    const now = new Date();
    ...
    if (!existingMeetings.length) {
      await db.execute(
        ...
        [meetingId, project.workspace_id, PROJECT_ID, MEETING_TITLE,
      ...
      await db.execute(
        `UPDATE meetings SET status = 'IN_PROGRESS', actual_start_time = ?, actual_end_time = NULL,
         end_time = ?, auto_completed = 0, mongo_summary_id = NULL, updated_at = NOW() WHERE id = ?`,
        [now, endTime, meetingId],
      );
    ...
    for (const [index, member] of selectedMembers.entries()) {
      await db.execute(
        `INSERT INTO meeting_participants
         ...
         VALUES (?, ?, ?, ?, 1, NOW(), NOW())`,
        [randomUUID(), meetingId, member.user_id, index === 0 ? 'HOST' : 'PARTICIPANT'],
      );
    }

    const collection = mongoose.connection.collection('meeting_transcripts');
