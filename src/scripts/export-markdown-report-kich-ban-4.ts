...
function generateMarkdownReport() {
  const resultsPath = resolve('datasets', 'project-assistant-evaluation', 'v1', 'openai_live_eval_results.json');
  const rawData = JSON.parse(readFileSync(resultsPath, 'utf8'));
...
`;
  ...
  for (const r of results) {
    const sourcesStr = r.sources.map((s: any) => `\`[${s.type}] ${s.label}\``).join('<br>');
  ...
  for (const r of results) {
    ...
    if (r.suggestedQuestions && r.suggestedQuestions.length > 0) {
      ...
      r.suggestedQuestions.forEach((q: string, idx: number) => {
        md += `  ${idx + 1}. *${q}*\n`;
      });
    }
  ...
  const outputPath = resolve('..', 'docs', 'KET_QUA_THUC_NGHIEM_PROJECT_AI_ASSISTANT_KICH_BAN_4.md');
