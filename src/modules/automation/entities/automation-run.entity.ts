...
export class AutomationRun {
  ...
  @Index() @Column({ name: 'rule_id', type: 'char', length: 36 }) ruleId: string;
  @Column({ name: 'task_id', type: 'char', length: 36, nullable: true }) taskId: string | null;
  @Index({ unique: true }) @Column({ name: 'execution_key', length: 220 }) executionKey: string;
