/**
 * Template email dang HTML.
 *
 * Client email (Gmail, Outlook) khong ho tro CSS ngoai va rat han che voi CSS
 * hien dai, nen tat ca style phai viet inline va layout dua tren the div don
 * gian. Day la ly do file nay khong dung chung style voi frontend.
 */
...
/** Doi xuong dong thuong thanh <br> de giu dinh dang nguoi dung da nhap. */
...
export const buildOtpMail = (params: {
  fullName: string;
  otp: string;
...
export const buildHandoverSubmittedMail = (
  params: HandoverMailParams & {
...
export const buildHandoverAcceptedMail = (
  params: HandoverMailParams,
): MailContent => {
  const body = `
    <p style="margin:0;font-size:15px;line-height:23px;color:#172b4d;">
...
export const buildHandoverRejectedMail = (
  params: HandoverMailParams & { reason: string },
): MailContent => {
  const body = `
    <p style="margin:0;font-size:15px;line-height:23px;color:#172b4d;">
      Xin chao <strong>${escapeHtml(params.recipientName)}</strong>,
...
export const buildHandoverChangesRequestedMail = (
  params: HandoverMailParams & { reason: string },
): MailContent => {
  const body = `
    <p style="margin:0;font-size:15px;line-height:23px;color:#172b4d;">
...
/** Danh sach gach dau dong, tra ve rong neu khong co muc nao. */
const bulletList = (label: string, items: string[]) => {
  ...
  const listItems = items
    .map(
      (item) =>
        `<li style="margin:0 0 6px;font-size:14px;line-height:21px;color:#172b4d;">${toHtmlParagraph(item)}</li>`,
    )
    .join('');
...
export const buildTeamReportApprovedMail = (params: {
  recipientName: string;
  approverName: string;
  projectName: string;
  reportDate: string;
