...
export class OtpService {
  ...
  async saveSecurityChallenge(
    ...
    const otpHash = await bcrypt.hash(otp, OTP_SALT_ROUNDS);
    await this.redis.set(
      `auth:${purpose}:otp:${email.toLowerCase()}`,
  ...
  async verifySecurityChallenge(
    ...
    if (await bcrypt.compare(otp, stored.otpHash)) {
      await this.redis.del(key);
      return { status: 'OK' as const, data: stored.data };
    }
    stored.attempts += 1;
    ...
    return {
      ...
      remainingAttempts: OTP_MAX_ATTEMPTS - stored.attempts,
    };
  }
