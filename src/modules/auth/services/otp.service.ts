...
export class OtpService {
  ...
  async saveSecurityChallenge(purpose: 'reset' | 'mfa', email: string, otp: string, data: Record<string, unknown> = {}) {
    const otpHash = await bcrypt.hash(otp, OTP_SALT_ROUNDS);
    await this.redis.set(`auth:${purpose}:otp:${email.toLowerCase()}`, JSON.stringify({ otpHash, attempts: 0, data }), 'EX', OTP_TTL_SECONDS);
  }

  async verifySecurityChallenge(purpose: 'reset' | 'mfa', email: string, otp: string) {
    const key = `auth:${purpose}:otp:${email.toLowerCase()}`;
    const raw = await this.redis.get(key);
    if (!raw) return { status: 'NOT_FOUND' as const };
    const stored = JSON.parse(raw) as { otpHash: string; attempts: number; data: Record<string, unknown> };
    if (stored.attempts >= OTP_MAX_ATTEMPTS) { await this.redis.del(key); return { status: 'TOO_MANY_ATTEMPTS' as const }; }
    if (await bcrypt.compare(otp, stored.otpHash)) { await this.redis.del(key); return { status: 'OK' as const, data: stored.data }; }
    stored.attempts += 1;
    const ttl = await this.redis.ttl(key);
    if (stored.attempts >= OTP_MAX_ATTEMPTS) { await this.redis.del(key); return { status: 'TOO_MANY_ATTEMPTS' as const }; }
    await this.redis.set(key, JSON.stringify(stored), 'EX', Math.max(1, ttl));
    return { status: 'INVALID' as const, remainingAttempts: OTP_MAX_ATTEMPTS - stored.attempts };
  }
