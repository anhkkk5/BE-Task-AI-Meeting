...
export class AiProviderService {
  ...
  private isRemoteProvider(provider: string) {
  ...
  private generateMeetingSummaryMockResponse(
    ...
    const memberPoints = new Map<string, string[]>();
    for (const line of workLines.filter((item) =>
      this.isWorkRelevantMeetingLine(item),
    )) {
      const [speaker, ...contentParts] = line.split(':');
      const content = contentParts.join(':').trim();
      if (!content || !speaker.trim()) continue;
      const current = memberPoints.get(speaker.trim()) ?? [];
      if (current.length < 2) current.push(content);
