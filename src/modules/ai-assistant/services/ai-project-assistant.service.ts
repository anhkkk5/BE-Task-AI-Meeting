...
export class AiProjectAssistantService {
  ...
  async ask(
    ...
    let output = fallback;
    if (
      !this.isUsageHelpQuestion(dto.question) &&
  ...
  private isOutOfScopeQuestion(question: string) {
    ...
    const asksForUnrelatedBuild =
      /(?:code|viết|lập trình|tạo|xây).*(?:thuật (?:toán|tón)|mã nguồn|website|web|app|ứng dụng|game|bot)|(?:thuật (?:toán|tón)|mã nguồn|website|web|app|ứng dụng|game|bot).*(?:code|viết|lập trình|tạo|xây)/iu.test(
        normalized,
      );
    const unrelatedTopic =
      /(?:nấu ăn|thời tiết|giải trí|tình yêu|xem bói|dịch thuật|giải bài|làm bài tập)/iu.test(
        normalized,
      );
