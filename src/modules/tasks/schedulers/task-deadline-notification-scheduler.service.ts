...
export class TaskDeadlineNotificationSchedulerService implements OnApplicationBootstrap {
  private readonly logger = new Logger(
    TaskDeadlineNotificationSchedulerService.name,
  );
  ...
  onApplicationBootstrap() {
    ...
    const cronTime = this.configService.get<string>(
      'TASK_DEADLINE_NOTIFICATION_CRON',
      '0 0 * * * *',
    );
    ...
    const job = CronJob.from({
      ...
      errorHandler: (error) =>
        this.logger.error('Task deadline scheduler failed', error),
    });
  ...
  async run(now = new Date()) {
    ...
    const tasks =
      await this.tasksRepository.findDueNotificationCandidates(tomorrow);
    let sent = 0;
  ...
  private enabled() {
    const value = this.configService.get<string | boolean>(
      'TASK_DEADLINE_NOTIFICATION_ENABLED',
    );
    return (
      value === undefined ||
      value === true ||
