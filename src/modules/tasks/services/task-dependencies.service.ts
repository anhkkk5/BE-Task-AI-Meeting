...
export class TaskDependenciesService {
  ...
  async create(
    ...
    await this.taskAccessService.assertTaskInProject(
      dto.targetTaskId,
      projectId,
    );
    if (sourceTaskId === dto.targetTaskId)
      throw new BadRequestException('Task can not depend on itself');
    if (
    ...
    if (
      [TaskDependencyType.Blocks, TaskDependencyType.DependsOn].includes(
        dto.type,
      )
    ) {
  ...
  private async assertNoCycle(
    ...
    const edges = (await this.repository.findByProject(projectId)).filter(
      (item) =>
        [TaskDependencyType.Blocks, TaskDependencyType.DependsOn].includes(
          item.type,
        ),
    ...
    edges.forEach((item) =>
      item.type === TaskDependencyType.Blocks
