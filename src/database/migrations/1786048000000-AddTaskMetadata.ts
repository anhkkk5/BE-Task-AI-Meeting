...
export class AddTaskMetadata1786048000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'ALTER TABLE `tasks` ADD `acceptance_criteria` text NULL',
    );
    await queryRunner.query(
      'ALTER TABLE `tasks` ADD `reporter_id` varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL',
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      'ALTER TABLE `tasks` ADD CONSTRAINT `FK_tasks_reporter_id` FOREIGN KEY (`reporter_id`) REFERENCES `users`(`id`) ON DELETE SET NULL',
    );
    await queryRunner.query(
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `tasks` DROP FOREIGN KEY `FK_tasks_reporter_id`',
    );
