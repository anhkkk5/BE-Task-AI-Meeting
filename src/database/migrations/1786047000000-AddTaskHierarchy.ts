...
export class AddTaskHierarchy1786047000000 implements MigrationInterface {
  ...
  async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      "ALTER TABLE `tasks` ADD `task_type` enum('EPIC','STORY','TASK','BUG','SUBTASK') NOT NULL DEFAULT 'TASK'",
    );
    await queryRunner.query(
      'ALTER TABLE `tasks` ADD `parent_id` varchar(36) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NULL',
    );
    await queryRunner.query(
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      'ALTER TABLE `tasks` DROP FOREIGN KEY `FK_tasks_parent_id`',
    );
